...
///
/// Not all events should be broadcasted through the [InqvineEventBus] but only those of
/// general interest.
///
/// Events are normal Dart objects. By specifying a class, listeners can
/// filter events.
///
class InqvineEventBus {
  StreamController _streamController;

  /// Controller for the event bus stream.
  StreamController get streamController => _streamController;

  /// Creates an [InqvineEventBus].
  ///
  ...
  /// completed.
  InqvineEventBus({bool sync = false}) : _streamController = StreamController.broadcast(sync: sync);
  ...
  InqvineEventBus.customController(StreamController controller) : _streamController = controller;
  ...
  ///
  Stream<T> on<T>() => streamController.stream.where((event) => event is T).cast<T>();
